...
const Button = ({ label, size = 'medium', type, variant = 'primary', onClick = () => { } }) => {
    ...
    return (
        <button
            className={`button ${sizeClasses[size]} ${variantClasses[variant]} rounded-full border font-inter-semibold uppercase text-white text-lg md:text-xl  line-clamp-1 overflow-hidden relative shadow-md`}
            onClick={onClick}
            type={type}
        >
            {label}
